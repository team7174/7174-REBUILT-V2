...
class CommandSwerveDrivetrain : public frc2::SubsystemBase,
   ...
   * HeadingController. See the documentation of
   * swerve::requests::SysIdSwerveRotation for info on importing the log to
   * SysId.
   */
  frc2::sysid::SysIdRoutine m_sysIdRoutineRotation{
      frc2::sysid::Config{
          /* This is in radians per second², but SysId only supports "volts per
             second" */
 ...
 public:
  ...
  CommandSwerveDrivetrain(
      ...
      : TunerSwerveDrivetrain{driveTrainConstants, odometryUpdateFrequency,
                              modules...} {
    if (utils::IsSimulation()) {
      StartSimThread();
    }
  ...
  /**
   * \brief Returns a command that applies the specified control request to this
   * swerve drivetrain.
   *
  ...
  template <typename RequestSupplier>
    requires std::is_lvalue_reference_v<
             ...
             requires(RequestSupplier req, TunerSwerveDrivetrain& drive) {
  ...
  /**
   * \brief Runs the SysId Quasistatic test in the given direction for the
   * routine specified by m_sysIdRoutineToApply.
   *
  ...
  void AddVisionMeasurement(frc::Pose2d visionRobotPose,
    ...
    TunerSwerveDrivetrain::AddVisionMeasurement(
        std::move(visionRobotPose), utils::FPGAToCurrentTime(timestamp));
  }
   ...
   * empty).
   */
  std::optional<frc::Pose2d> SamplePoseAt(
      units::second_t timestamp) const override {
    return _drivetrain.SamplePoseAt(utils::FPGAToCurrentTime(timestamp));
  }

  /**
   * \brief Seeds the drivetrain rotation (gyro heading) from a vision-derived
   * rotation without disturbing the current XY translation estimate.
   *
   * Calls ResetRotation on the underlying SwerveDrivetrain, which resets
   * both the internal Pigeon2 offset and the pose estimator rotation so that
   * the field-centric drive and odometry stay consistent.
   *
   * \param rotation The new heading to apply (field-relative, WPILib Blue
   * origin).
   */
